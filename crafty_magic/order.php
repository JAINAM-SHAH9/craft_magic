<?php 
	...
	{
		echo '<script>alert("you are not logged-in yet please sign-in to visit the site.😊😊")</script>';
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navbars-rs-food">
					<ul class="navbar-nav ml-auto">
						...
						<li class="nav-item active"><li class="nav-item active"><a class="nav-link" href="order.html">Orders</a></li>
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="heading-title text-center">
						<h1 style="text-size-adjust: 50px"><label for="">Your orders</label></h1>
						<hr>
						<?php 
							$email=$_SESSION["logindatabase"];
							...
							while($order=mysqli_fetch_assoc($fire_select))
							{
								$name=$order["prod"];
								echo $name;
								$select_prod="SELECT * FROM product WHERE name='$name';";
								$fire_select_prod=mysqli_query($con,$select_prod);
								$id=$order["id"];
								if(!$fire_select_prod)
								{
									echo mysqli_error($con);
								}
								$product=mysqli_fetch_assoc($fire_select_prod);
								
								$img_src="./product_img/".$product["img"];
							
								$dis=$product["dis"];
								$pd_price=$product["price"];
								?>
								<h1><label for=""><?php echo strtoupper($name); ?></label></h1>
									...
									<form action="" method="post">
										<input type="submit" value="cancle" name="can">
							...
							{
								$id_to_de=$_POST["id"];
								$delete="DELETE FROM orders WHERE id='$id_to_de';";
								$fire_delete=mysqli_query($con,$delete);
								if(!$fire_delete)
								{
									echo mysqli_error($con);
								}
								echo '<script>alert("your order has been cancled.🥺😔")</script>';
